...
{
	public partial class SavePoll : UIUserControlBase
	{
		...
		protected void btnSave_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (dtSmsParserOptions.Rows.Count == 0)
					throw new Exception(Language.GetString("SelcectPollOptions"));
				#endregion
				...
				{
					case "insert":
						if (!Facade.SmsParser.InsertPoll(smsParser, dtSmsParserOptions))
							throw new Exception(Language.GetString("ErrorRecord"));

						break;
					case "edit":
						smsParser.SmsParserGuid = SmsParserGuid;
						if (!Facade.SmsParser.UpdatePoll(smsParser, dtSmsParserOptions))
							throw new Exception(Language.GetString("ErrorRecord"));
						break;
				}

				Response.Redirect(string.Format("/PageLoader.aspx?c={0}", Helper.Encrypt((int)Arad.SMS.Gateway.Business.UserControls.UI_SmsParsers_Polls_Poll, Session)));
			}
			catch (Exception ex)
			{
		...
		{
			return (int)Arad.SMS.Gateway.Business.UserControls.UI_SmsParsers_Polls_SavePoll;
		}

		protected override string GetUserControlTitle()
		{
			return Language.GetString(Business.UserControls.UI_SmsParsers_Polls_SavePoll.ToString());
		}
	}
}
