...
{
	public partial class GiveBackCreditService : ServiceBase
	{
		private GiveBackThread giveBackThread;
		private ConfirmBulkThread confirmBulkThread;
		private GarbageCollectorThread gcThread;

		public GiveBackCreditService()
		{
			InitializeComponent();
		}
		...
		protected override void OnStop()
		{
			try
			{
				if (giveBackThread != null)
					giveBackThread.Stop();

				if (confirmBulkThread != null)
					confirmBulkThread.Stop();
			...
			{
				LogController<ServiceLogs>.LogInFile(ServiceLogs.GiveBackCredit, string.Format("\r\n------------------OnStop-------------------------------"));
				LogController<ServiceLogs>.LogInFile(ServiceLogs.GiveBackCredit, string.Format("On Stop GiveBackCreditService : {0}", ex.Message));
				LogController<ServiceLogs>.LogInFile(ServiceLogs.GiveBackCredit, string.Format("\r\n--------------------------------------------------------"));
			}
		}
		...
		protected override void OnShutdown()
		{
			...
			{
				LogController<ServiceLogs>.LogInFile(ServiceLogs.GiveBackCredit, string.Format("\r\n------------------OnShutdown-------------------------------"));
				LogController<ServiceLogs>.LogInFile(ServiceLogs.GiveBackCredit, string.Format("On Shutdown GiveBackCreditService : {0}", ex.Message));
				LogController<ServiceLogs>.LogInFile(ServiceLogs.GiveBackCredit, string.Format("\r\n--------------------------------------------------------"));
			}
		}
	}
