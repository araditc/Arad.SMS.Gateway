...
function (a, b) {
		...
		if (a.fn.ace_ajax) {
			...
			a(window).on("error.ace_ajax", function () {
				a("[data-ajax-content=true]").each(function () {
					var b = a(this);
					b.ace_ajax("working") && (window.Pace && Pace.running && Pace.stop(),
					b.ace_ajax("stopLoading", !0))
				})
			})
		}
	}
	...
	ace.demo.functions.navbarHelpers = function () {
		...
		a(document).on("click", ".dropdown-navbar .nav-tabs", function (b) {
			...
			var c;
			b.target;
			(c = a(b.target).closest("[data-toggle=tab]")) && c.length > 0 && (c.tab("show"),
			b.preventDefault(),
			a(window).triggerHandler("resize.navbar.dropdown"))
		})
	}
	,
	...
	ace.demo.functions.someBrowserFix = function () {
		...
		ace.vars.ios_safari && a(document).on("ace.settings.ios_fix", function (b, c, d) {
			"navbar_fixed" == c && (a(document).off("focus.ios_fix blur.ios_fix", "input,textarea,.wysiwyg-editor"),
			1 == d && a(document).on("focus.ios_fix", "input,textarea,.wysiwyg-editor", function () {
	...
	ace.demo.functions.bsCollapseToggle = function () {
		a(document).on("hide.bs.collapse show.bs.collapse", function (b) {
			...
			0 != d.length && d.find(ace.vars[".icon"]).each(function () {
				...
				return (e = d.attr("data-icon-show")) ? f = d.attr("data-icon-hide") : (c = d.attr("class").match(/fa\-(.*)\-(up|down)/)) && (e = "fa-" + c[1] + "-down",
				f = "fa-" + c[1] + "-up"),
				e ? ("show" == b.type ? d.removeClass(e).addClass(f) : d.removeClass(f).addClass(e),
				!1) : void 0
			})
		})
	}
	...
	ace.demo.functions.smallDeviceDropdowns = function () {
		...
		function c(b) {
			if (b = b || a(this).find("> .dropdown-menu"),
			b.length > 0 && (b.css({
				width: "",
				height: "",
				left: "",
...
function (a, b) {
	function c(b, c) {
		...
		g = f.get(0)) {
			...
			, A = function (b) {
				if (!v && j.sidebar_fixed && (f = l.find(".nav-list"),
				h = l.find(".sidebar-toggle").eq(0),
				i = l.find(".sidebar-shortcuts").eq(0),
				g = f.get(0))) {
					...
					j.settings.include_shortcuts && 0 != i.length && (f.parent().prepend(i).wrapInner("<div />"),
					f = f.parent()),
					j.settings.include_toggle && 0 != h.length && (f.append(h),
					f.closest(".nav-wrap").addClass("nav-wrap-t")),
					f.css({
						position: "relative"
					}),
					...
					if (f.on("ace_drag.nav", function (b) {
							...
							if ("up" == b.direction || "down" == b.direction) {
									...
									Math.abs(d) > 2 && (d = 2 * d),
									0 != d && (c.scrollTop = c.scrollTop + d,
									g.style.top = -1 * c.scrollTop + "px")
					}
					}),
					...
					if (v = !0,
					1 == b && (j.reset(),
					n && j.scroll_to_active(),
					n = !1),
					"number" == typeof j.settings.smooth_scroll && j.settings.smooth_scroll > 0 && (f.css({
					...
					ace.vars.android) {
						...
						2 > k && (window.scrollTo(k, 0),
						setTimeout(function () {
							j.reset()
						}, 20));
						var m, o = ace.helper.winHeight();
						a(window).on("scroll.ace_scroll", function () {
							j.is_scrolling && u.is_active() && (m = ace.helper.winHeight(),
							m != o && (o = m,
							j.reset()))
						})
					}
			...
			this.scroll_to_active = function () {
				if (u && u.is_active())
					try {
						...
						b.minimized && !b.collapsible ? a = c.find("> .active") : (a = f.find("> .active.hover"),
						0 == a.length && (a = f.find(".active:not(.open)")));
						var d = a.outerHeight();
						c = c.get(0);
						for (var e = a.get(0) ; e != c;)
							...
							e = e.parentNode;
						var h = d - p.height();
						h > 0 && (g.style.top = -h + "px",
						q.scrollTop(h))
					} catch (i) { }
			}
			,
			this.reset = function (a) {
				if (a === !0 && (this.sidebar_fixed = e(b, "fixed")),
				!this.sidebar_fixed)
			...
			this.disable = function () {
				...
				u.disable()),
				parseInt(g.style.top) < 0 && j.settings.smooth_scroll && a.support.transition.end ? f.one(a.support.transition.end, function () {
			...
			l.on("hidden.ace.submenu.sidebar_scroll shown.ace.submenu.sidebar_scroll", ".submenu", function (a) {
				a.stopPropagation(),
				o.get("minimized") || (j._reset(),
				"shown" == a.type && j.set_hover())
			}),
			A(!0)
		}
	}
...
function (a, b) {
	function c(b, c) {
		...
		if (c) {
			...
			g = parseInt(g),
			d.css("min-height", g),
			e ? (f && d.ace_scroll("disable"),
			d.css("max-height", g).addClass("overflow-scroll")) : (f ? d.ace_scroll("update", {
				size: g,
				mouseWheelLock: !0,
				lockAnyway: !0
			}) : d.ace_scroll({
	...
	var d = function (b, d) {
		...
		this.toggle = function (a, b, c, d) {
			...
			if (b || (b = e.find("> .widget-header a[data-action=collapse]").eq(0),
			0 == b.length && (b = null)),
			b) {
				g = b.find(ace.vars[".icon"]).eq(0);
				var j, k = null, l = null;
				(k = g.attr("data-icon-show")) ? l = g.attr("data-icon-hide") : (j = g.attr("class").match(/fa\-(.*)\-(up|down)/)) && (k = "fa-" + j[1] + "-down",
				l = "fa-" + j[1] + "-up")
			}
			var c = "undefined" == typeof c ? 250 : c
				, d = "undefined" == typeof d ? 200 : d;
			"show" == h ? (g && g.removeClass(k).addClass(l),
			f.hide(),
		...
		this.fullscreen = function (a) {
			var b = this.$box.find("> .widget-header a[data-action=fullscreen]").find(ace.vars[".icon"]).eq(0)
				, d = null
				, e = null;
			(d = b.attr("data-icon1")) ? e = b.attr("data-icon2") : (d = "fa-expand",
			e = "fa-compress");
			var f = this.$box.hasClass("fullscreen")
				, g = a !== !0 && a !== !1;
			a === !0 || g && !f ? (b.removeClass(d).addClass(e),
			this.$box.addClass("fullscreen"),
			c(this.$box, !0)) : (a === !1 || g && f) && (b.addClass(d).removeClass(e),
	...
	a(document).on(ace.click_event + ".ace.widget", ".widget-header a[data-action]", function (b) {
		...
		if (0 != e.length && !e.hasClass("ui-sortable-helper")) {
			...
			if ("collapse" == g) {
				...
				if (e.trigger(h = a.Event(i + ".ace.widget")),
				...
				var h;
				if (e.trigger(h = a.Event("close.ace.widget")),
				h.isDefaultPrevented())
					return;
				f.close()
			} else if ("reload" == g) {
				c.blur();
				var h;
				if (e.trigger(h = a.Event("reload.ace.widget")),
				h.isDefaultPrevented())
					return;
				f.reload()
			} else if ("fullscreen" == g) {
				var h;
				if (e.trigger(h = a.Event("fullscreen.ace.widget")),
				h.isDefaultPrevented())
					return;
				f.fullscreen()
			} else
	...
	ace.settingFunction = {
		navbar_fixed: function (b, c, d, e) {
			if (ace.vars.very_old_ie)
				return !1;
			var b = b || "#navbar";
			if ("string" == typeof b && (b = a(b).get(0)),
			!b)
				return !1;
			var f, c = c || !1, d = "undefined" != typeof d ? d : !0;
			if (a(document).trigger(f = a.Event("presettings.ace"), ["navbar_fixed", c, b, d]),
		...
		sidebar_fixed: function (b, c, d, e) {
			...
			!b)
				return !1;
			var f, c = c || !1, d = "undefined" != typeof d ? d : !0;
			if (a(document).trigger(f = a.Event("presettings.ace"), ["sidebar_fixed", c, b, d]),
			f.isDefaultPrevented())
				return !1;
			e !== !1 && (c ? ace.settingFunction.navbar_fixed(null, !0, d) : ace.settingFunction.breadcrumbs_fixed(null, !1, d));
			var g = a("#menu-toggler");
			c ? (a(b).addClass("sidebar-fixed"),
			g.addClass("fixed")) : (a(b).removeClass("sidebar-fixed"),
			g.removeClass("fixed")),
			d && (ace.settings.saveState(b, "class", "sidebar-fixed", c),
			0 != g.length && ace.settings.saveState(g[0], "class", "fixed", c)),
		...
		breadcrumbs_fixed: function (b, c, d, e) {
			...
			f.isDefaultPrevented() ? !1 : (c && e !== !1 && ace.settingFunction.sidebar_fixed(null, !0, d),
		...
		main_container_fixed: function (b, c, d) {
			...
			!b)
				return !1;
			var e;
			if (a(document).trigger(e = a.Event("presettings.ace"), ["main_container_fixed", c, b, d]),
			e.isDefaultPrevented())
				return !1;
			var f = a("#navbar-container");
			c ? (a(b).addClass("container"),
			a(f).addClass("container")) : (a(b).removeClass("container"),
			...
			navigator.userAgent.match(/webkit/i) && (a("#sidebar").toggleClass("menu-min"),
			setTimeout(function () {
...
function (a, b) {
	...
	var c = function () {
		function b(b) {
			...
			f ? (e.find(".scroll-hz").addClass("make-ltr").find(".scroll-content").wrapInner('<div class="make-rtl" />'),
			a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("changeDir", "right")) : (e.find(".scroll-hz").removeClass("make-ltr").find(".make-rtl").children().unwrap(),
			a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("changeDir", "left")),
