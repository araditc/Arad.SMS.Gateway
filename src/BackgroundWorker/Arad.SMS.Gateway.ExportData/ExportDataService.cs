	...
	public partial class ExportDataService : ServiceBase
	{
		...
		protected override void OnStart(string[] args)
		{
			try
			{
				...
				//Thread.Sleep(30000);
				exportThread = new ExportThread(5 * 1000);
			...
			catch (Exception ex)
			{
				...
				LogController<ServiceLogs>.LogInFile(ServiceLogs.ExportData, string.Format("On Start ExportDataService : {0}", ex.Message));
				LogController<ServiceLogs>.LogInFile(ServiceLogs.ExportData, string.Format("\r\n--------------------------------------------------------"));
			}
		}
		...
		protected override void OnShutdown()
		{
			...
			{
				LogController<ServiceLogs>.LogInFile(ServiceLogs.ExportData, string.Format("\r\n------------------OnShutdown-------------------------------"));
				LogController<ServiceLogs>.LogInFile(ServiceLogs.ExportData, string.Format("On Shutdown ExportDataService : {0}", ex.Message));
				LogController<ServiceLogs>.LogInFile(ServiceLogs.ExportData, string.Format("\r\n--------------------------------------------------------"));
			}
		}
	}
