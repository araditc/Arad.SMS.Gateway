...
function messageBox(messageText, messageBoxTitle, messageBoxStyle, callbackFunction) {
	var result = "";

	if (messageBoxTitle == undefined || messageBoxTitle == "")
		messageBoxTitle = '<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("Alert")%>';

	var rootWindowElement = findRootParrentWindow(false);

	if (messageBoxStyle == undefined || messageBoxStyle.toLowerCase() == "alert")
		rootWindowElement.$.prompt(messageText,
																{
																	title: messageBoxTitle,
																	prefix: 'jqiAsr',
																	buttons: { '<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("Ok")%>': true }
																});
																	...
																	rootWindowElement.$.prompt(messageText,
																															{
																																title: messageBoxTitle,
																																prefix: 'jqiAsr',
																																callback: function (event, value, message, formVlues) {
																																	if (callbackFunction != undefined && value != undefined)
																																		callbackFunction(value);
																																},
																																buttons: {
																																	'<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("Ok")%>': true,
																			'<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("Cancel")%>': false
																		}
																});
														}
														...
														$(document).ready(function () {
															...
															$(".numberInput").blur(function (event) {
																...
																if (!eval(srcEl.getAttribute("isValid"))) {
																	...
																	messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("EnteredNumberIsNotValid")%>');
			//srcEl.focus();
			return false;
		} else {
			if (eval(srcEl.getAttribute("isvalid")))
	...
	$(".hourInput").blur(function (event) {
		...
		if (srcEl.value == "") {
			...
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("CheckEmptyHourInput") %>');
		}
		else {
			if (hour > 23) {
				//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("FailHourInput") %>');
				srcEl.style.backgroundColor='#f2dede';
				srcEl.style.border = "1px solid red";
				srcEl.value = "00";
			}
	...
	$(".minuteInput").blur(function (event) {
		...
		if (srcEl.value == "") {
			srcEl.style.border = "1px solid red";
			srcEl.style.backgroundColor='#f2dede';
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("CheckEmptyMinuteInput") %>');
		}
		else {
			if (minute > 59) {
				//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("FailMinuteInput") %>');
				srcEl.style.border = "1px solid red";
				srcEl.style.backgroundColor='#f2dede';
				srcEl.value = "00";
			}
	...
	$(".mobileNumberInput").blur(function (event) {
		...
		if (!eval(srcEl.getAttribute("isValid"))) {
			...
			srcEl.style.border = "1px solid red";
			srcEl.style.backgroundColor='#f2dede';
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("MobileNumberIsNotValid")%>');
			//srcEl.focus();
	...
	$(".mobileNumberInputList").blur(function (event) {
		...
		$("#txtCorrectNumber")[0].value = '<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("NumberOfValidMobileNumbers")%>' + " : " + countCorrectMobileNumbers;
		...
		if (!eval(srcEl.getAttribute("isValid"))) {
			...
			srcEl.style.border = "1px solid red";
			srcEl.style.backgroundColor='#f2dede';
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("MobileNumbersIsNotValid")%>');
	...
	$(".emailInput").blur(function (event) {
		...
		if (!invalidEmail) {
			...
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("EmailIsNotValid")%>');
	...
	$(".postalCodeInput").blur(function (event) {
		...
		if (postalCode.length != 10) {
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("FailPostalCodeInput")%>');
			srcEl.style.border = "1px solid red";
			srcEl.style.backgroundColor='#f2dede';
			srcEl.value = "00";
		}
	...
	$(".natinalCodeInput").blur(function (event) {
		...
		if (nationalCode.length != 10) {
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("FailNationalCodeInput")%>');
			srcEl.style.border = "1px solid red";
			srcEl.style.backgroundColor='#f2dede';
			srcEl.value = "00";
		}
	...
	$(".phoneNumberInput").blur(function (event) {
		...
		if (phoneNumberInput.length > 10) {
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("FailphoneNumberInput")%>');
			srcEl.style.border = "1px solid red";
			srcEl.style.backgroundColor='#f2dede';
			srcEl.value = "00";
		}
	...
	$(".emailInputList").blur(function (event) {
		...
		$("#txtCorrectEmail")[0].value = '<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("NumberOfValidEmails")%>' + " : " + countCorrectEmails;
		...
		if (!eval(srcEl.getAttribute("isValid"))) {
			...
			srcEl.style.border = "1px solid red";
			srcEl.style.backgroundColor='#f2dede';
			//messageBox('<%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("EmailsIsNotValid")%>');
