<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SaveCompetition.ascx.cs" Inherits="Arad.SMS.Gateway.Web.UI.SmsParsers.Competitions.SaveCompetition" %>
...
<script type="text/javascript">
	function addKeyword() {
		...
		var isCorrect = $("#chbIsCorrect").attr("checked") == 'checked' ? true : false;

		addRow(keywordTitle, keyword, phoneBook, isCorrect);
	}

	function addRow(keywordTitle, keyword, phoneBook, isCorrect) {
		gridKeywords.AddRow({
			Title: keywordTitle,
			Key: keyword,
			ReferenceGuid: phoneBook,
			IsCorrect: isCorrect ? "<span class='fa fa-2x fa-check green'></span>" : "<span class='fa fa-2x fa-times red'></span>",
	...
	function save() {
		if (!validateRequiredFields('saveCompetition'))
			return false;
...
<div class="row">
	<div class="col-md-5">
		<div class="width-100 label label-info label-xlg arrowed-in arrowed-in-right"><i class="ace-icon fa fa-circle light-red"></i><%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("CompetitionSpecifications")%></div>
		<div class="form-horizontal" role="form">
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<asp:DropDownList ID="drpSenderNumber" class="form-control input-sm" validationSet="saveCompetition" isRequired="true" runat="server"></asp:DropDownList>
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("CompetitionName")%></label>
				<div class="col-sm-9">
					<asp:TextBox ID="txtTitle" class="form-control input-sm" validationSet="saveCompetition" isRequired="true" runat="server"></asp:TextBox>
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("StartDate")%></label>
				<div class="col-sm-9">
					<SMS:DateTimePicker ID="dtpStartDate" IsRequired="true" ValidationSet="saveCompetition" runat="server"></SMS:DateTimePicker>
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("EndDate")%></label>
				<div class="col-sm-9">
					<SMS:DateTimePicker ID="dtpEndDate" IsRequired="true" ValidationSet="saveCompetition" runat="server"></SMS:DateTimePicker>
				</div>
			</div>
			<div class="form-group">
	...
	<div class="col-md-7">
		...
		<div class="form-inline" role="form" style="margin-right: 13px;">
			...
			<div class="form-group">
				...
				<div>
					<label class="block">
						<input name="form-field-checkbox" type="checkbox" class="ace" id="chbIsCorrect">
						<span class="lbl"><%=Arad.SMS.Gateway.GeneralLibrary.Language.GetString("CorrectOption") %></span>
					</label>
				</div>
			</div>
			<div class="form-group">
		...
		<div class="col-md-12">
			<GeneralTools:DataGrid runat="server" ID="gridKeywords" DefaultSortField="Type" ListCaption="OptionsList" ShowRowNumber="true" DisableNavigationBar="true" ListHeight="150">
				<Columns>
					<GeneralTools:DataGridColumnInfo FieldName="IsCorrect" Caption="CorrectOption" Sortable="false" Align="Center" CellWidth="50" FormattingMethod="BooleanOnOff" />
					<GeneralTools:DataGridColumnInfo FieldName="CorrectKey" Caption="CorrectOption" Hidden="true" Sortable="false" Align="Center" CellWidth="50" />
