...
(function (factory) {
	...
	$.fn.jqGrid = function (pin) {
		...
		return this.each(function () {
			...
			var p = $.extend(true, {
				...
				ExpandColumn: null,
				tree_root_level: 0,
				prmNames: { serializeObject: "serializeObject", c: "c", page: "page", rows: "rows", sort: "sidx", order: "sord", search: "_search", nd: "nd", id: "id", oper: "oper", editoper: "edit", addoper: "add", deloper: "del", subgridid: "id", npage: null, totalrows: "totalrows" },
				forceFit: false,
				gridstate: "visible",
				cellEdit: false,
				cellsubmit: "remote",
			...
			populate = function (npage) {
				if (!ts.grid.hDiv.loading) {
					...
					if (pN.c !== null) { prm[pN.c] = $(ts.grid.hDiv).parents("form:first").attr("action"); }
					if (pN.serializeObject !== null) { prm[pN.serializeObject] = ts.p.serializeObject; }
			...
			setPager = function (pgid, tp) {
				...
				if (ts.p.pgbuttons === true) {
					...
					pgl += "<td id='" + po[0] + "' class='ui-pg-button ui-corner-all' title='" + ($.jgrid.getRegional(ts, "defaults.pgfirst", ts.p.pgfirst) || "") + "'" + "><span class='ui-icon ace-icon fa fa-angle-double-left bigger-140'></span></td>";
					pgl += "<td id='" + po[1] + "' class='ui-pg-button ui-corner-all' title='" + ($.jgrid.getRegional(ts, "defaults.pgprev", ts.p.pgprev) || "") + "'" + "><span class='ui-icon ace-icon fa fa-angle-left bigger-140'></span></td>";
					pgl += pginp !== "" ? sep + pginp + sep : "";
					pgl += "<td id='" + po[2] + "' class='ui-pg-button ui-corner-all' title='" + ($.jgrid.getRegional(ts, "defaults.pgnext", ts.p.pgnext) || "") + "'" + "><span class='ui-icon ace-icon fa fa-angle-right bigger-140'></span></td>";
					pgl += "<td id='" + po[3] + "' class='ui-pg-button ui-corner-all' title='" + ($.jgrid.getRegional(ts, "defaults.pglast", ts.p.pglast) || "") + "'" + "><span class='ui-icon ace-icon fa fa-angle-double-right bigger-140'></span></td>";
				} else if (pginp !== "") {
					pgl += pginp;
				}
				if (dir === "ltr") {
				...
				case "select":
					...
					if (options.dataUrl != null) {
						...
						$.ajax($.extend({
							...
							type: "POST",
							dataType: "json",
							data: "{id:" + postData.id + "}",//$.isFunction(postData) ? postData.call($t, rowid, vl, String(options.name)) : postData,
	...
	$.jgrid.extend({
			...
			return this.each(function () {
				...
				buildRuleMenu = function (elem, left, top) {
					...
					for (i = 0 ; i < options.sopt.length; i++) {
						...
						if (ina !== -1) {
							...
							str += '<li class="ui-menu-item ' + selclass + '" role="presentation"><a class="ui-corner-all g-menu-item" tabindex="0" role="menuitem" value="' + p.odata[ina].oper + '" oper="' + p.operands[p.odata[ina].oper] + '"><table cellspacing="0" cellpadding="0" border="0"><tr><td width="25px" style="direction:ltr;">' + p.operands[p.odata[ina].oper] + '</td><td>' + p.odata[ina].text + '</td></tr></table></a></li>';
						}
					}
					str += "</ul>";
					$('body').append(str);
				...
				$.each($t.p.colModel, function (ci) {
					...
					if (this.search) {
						...
						$(elem).css({ width: "100%" }).addClass("ui-widget-content ui-corner-all " + cm.class);
		...
		searchGrid: function (searchDivID) {
			$("#"+searchDivID).modal('show');
			return true;
		},
		searchGrid1: function (p) {
			var regional = $.jgrid.getRegional(this[0], 'search');
			p = $.extend(true, {
					...
					if (o.search) {
						...
						.click(function () {
							if (!$(this).hasClass('ui-state-disabled')) {
								...
								//}
								$($t).jqGrid("searchGrid", o.searchDivID);
							}
							return false;
						}).hover(onHoverIn, onHoverOut);
